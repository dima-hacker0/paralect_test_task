...
const Repositories = (props) => {
  ...
  console.log(itemOffset + 1, "-", itemOffset + 4);

  const firstItem = itemOffset + 1;
  console.log(itemOffset);
  const lastItem =
    itemOffset + 4 <= repositories.length
      ? itemOffset + 4
      : repositories.length;
  ...
  return (
    <div className={classes.repositories}>
      ...
      <div className={classes.paginateButtons}>
        <div
          className={classes.numberItems}
        >{`${firstItem}-${lastItem} of ${repositories.length} items`}</div>
        <ReactPaginate
          breakLabel="..."
          nextLabel=">"
          ...
          pageLinkClassName={classes.linkPage}
        />
      </div>
    </div>
  );
